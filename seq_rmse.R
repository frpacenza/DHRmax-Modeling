# ---------------------------------------------------------------------------- #
# SEQUENTIAL RMSE ACROSS METHODS
# ---------------------------------------------------------------------------- #
# Import data and functions

load("~/Desktop/Project/predictors_df_with_target.RData")
source("~/Desktop/Project/functions.R")
...
corr_predictors <- corr_fs(predictors = predictors, target = target, n = 10)
corr_selected <- names(corr_predictors)
corr_res <- get_sequential_cv_rmse(selected = corr_selected, target = target, predictors = predictors)

enet_predictors <- enet_fs(predictors = predictors, target = target, n = 10)
enet_selected <- names(enet_predictors)
enet_res <- get_sequential_cv_rmse(selected = enet_selected, target = target, predictors = predictors)

rforest_predictors <- rforest_fs(predictors = predictors, target = target, n = 10)
rforest_selected <- names(rforest_predictors)
rforest_res <- get_sequential_cv_rmse(selected = rforest_selected, target = target, predictors = predictors)

plot_sequential_rmse(corr_res)
plot_sequential_rmse(enet_res)
plot_sequential_rmse(rforest_res)

generate_latex_table(corr_res, enet_res, rforest_res)
...
corr_predictors_M <- corr_fs(predictors = predictors_M, target = target_M, n = 10)
corr_selected_M <- names(corr_predictors_M)
corr_res_M <- get_sequential_cv_rmse(selected = corr_selected_M, target = target_M, predictors = predictors_M)

enet_predictors_M <- enet_fs(predictors = predictors_M, target = target_M, n = 10)
enet_selected_M <- names(enet_predictors_M)
enet_res_M <- get_sequential_cv_rmse(selected = enet_selected_M, target = target_M, predictors = predictors_M)

rforest_predictors_M <- rforest_fs(predictors = predictors_M, target = target_M, n = 10)
rforest_selected_M <- names(rforest_predictors_M)
rforest_res_M <- get_sequential_cv_rmse(selected = rforest_selected_M, target = target_M, predictors = predictors_M)

plot_sequential_rmse(corr_res_M)
plot_sequential_rmse(enet_res_M)
plot_sequential_rmse(rforest_res_M)

generate_latex_table(corr_res_M, enet_res_M, rforest_res_M)
